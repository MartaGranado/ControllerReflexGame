const buttons = ['cross', 'circle', 'square', 'triangle'];
const displayMap = {
    cross: '✕', circle: '〇', square: '☐', triangle: '△'
};
let currentButton = '';
...
function highlightButton(button) {
    ...
    document.getElementById('targetButton').textContent = displayMap[button];
    }
}
...
function checkGamepad() {
    ...
    if (gamepads[0]) {
        ...
        // Check if the connected controller is a PlayStation controller
        if (gp.id.toLowerCase().includes("playstation")) {
        ...
        // If the connected controller isn't a PlayStation controller, stop the game and alert the user
        if (gameRunning) {
            alert("Este juego solo acepta controladores PlayStation.");
            goToMenu();
            setTimeout(() => {
